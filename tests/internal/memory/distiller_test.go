...
func TestDistiller_MarksConsumedEpisodes(t *testing.T) {
	...
	// All 3 source episodes should remain with DistilledAt set (not deleted).
	for _, id := range []string{"e1", "e2", "e3"} {
		var found *memory.Memory
		for i := range store.mems {
			if store.mems[i].ID == id {
				found = &store.mems[i]
				break
			}
		}
		require.NotNil(t, found, "episode %s should still exist after distill", id)
		assert.NotNil(t, found.DistilledAt, "episode %s should have DistilledAt set", id)
	}
}
