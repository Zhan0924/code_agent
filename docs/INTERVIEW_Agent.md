...
2. **情景记忆（Episodic Memory）：项目级的业务知识补充**
   - **载体**：PostgreSQL (trajectories 表)。
   - **特点**：被动检索，按需召回。
   - **工程处理**：当 Agent 遇到任务时，会检索历史成功的执行轨迹（task → actions → result），将过往相似的经验作为“工作背景”动态插拔到短时记忆中。
3. **长期记忆（Semantic Memory）：跨越会话的知识沉淀**
   - **载体**：`HybridStore`（冷热分离存储：Redis + PostgreSQL/Qdrant）。
   - **特点**：主动提取、长久保存、跨项目复用。
   - **工程处理**：系统利用 `Extractor` 自动提取用户的偏好（Preference）和架构决策（Decision）。入库前通过 `ConflictResolver` 进行冲突消解。
...
只保留最近 10 条在 Redis Hot key，超出的自动归档到 Redis Cold key（+ 可选持久存储）。需要时按需召回。
...
### 11.2 基础问题：你们的长期记忆系统为什么采用 Hybrid Memory（冷热分离）架构？
...
### 11.3 进阶问题：长期记忆是如何从对话流水中自动提取的？（Extractor 原理）
...
### 11.4 深度问题：如果在入库时发现新提取的记忆与老记忆存在冲突，系统是如何处理的？
...
## 十二、安全边界与意图冲突处理（Safety & Intent Conflict Resolution）

### 12.1 深度问题：如果用户和 AI 对话生成的指令本身有冲突，系统应该怎么处理？
