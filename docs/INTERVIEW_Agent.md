...
2. **记忆机制 (Memory Management)**
   ...
   - **长期/情景记忆**：通过 RAG 等向量检索技术，持久化并按需召回过去的架构决策、业务知识或高频代码片段。
     ...
     > ```text
     > ┌─────────────────────────────────────────────────────────────────────────┐
     > │                 三层协同的立体记忆架构 (Memory Architecture)            │
     > └─────────────────────────────────────────────────────────────────────────┘
     > 
     >   [ 用户输入 / 当前任务 ]
     >            │
     ...
     >   │    - 作用：维持当前会话的流转，确保“手头活儿不出错”。          │
     >   │    - 痛点：Token 极易爆表。                                 │
     >   │    - 技术：基于 Token Budget 的滑动窗口 + AST-aware 裁剪。  │
     >   │           (将超大文件“挖空留签名”，实现物理级压缩)           │
     >   └────────────────────────┬───────────────────────────────────┘
     >                            │ (按需拉取背景)
     >                            ▼
     ...
     >   │    - 触发：在执行特定任务(如检索)时毫秒级召回。                 │
     >   │    - 技术：混合 RAG 引擎 (BM25 稀疏检索 + Qdrant 稠密检索)。 │
     >   │           (揪出高度相关的本地代码片段提供上下文)               │
     >   └────────────────────────┬───────────────────────────────────┘
     >                            │ (对话结束后，异步萃取提纯)
     >                            ▼
     ...
     >   │    - 触发：任务收尾时由专门的 Extractor 异步提取。            │
     >   │    - 技术：                                                │
     >   │      a. LLM 萃取器：提取用户偏好、架构决策、高频报错方案。      │
     >   │      b. HybridStore (冷热分离)：Redis(热) + pgvector(冷)。   │
     >   │      c. Conflict Resolver：基于 Jaccard 与 Embedding        │
     >   │         拦截并消解记忆冲突，防止知识精神分裂。                  │
     >   └────────────────────────────────────────────────────────────┘
     > ```
     >
     > **运行机制简述：**
     > - **短时记忆**负责决定当前的推理怎么往下走；
     > - 如果短时记忆觉得信息不够，向**情景记忆**求助，拉取项目背景代码；
     > - 当整个任务闭环后，**长期记忆**模块启动，将这次任务里的“坑”和“用户习惯”提纯存入数据库，化作 Agent 成长的一部分。
...
### 1.12 架构问题：ReAct 与 Chain-of-Thought、Tree-of-Thoughts 的本质区别是什么？各自适用场景？
...
### 1.13 架构问题：如何设计 Agent 的自我反思（Self-Reflection）机制？
...
### 1.14 架构问题：如何处理 ReAct 循环中的"幻觉"（Hallucination）问题？
...
### 1.15 面试官问："Agent 系统如何约束大模型幻觉？约束后还出现幻觉怎么办？"
...
### 4.1 核心架构问题：我们的 RAG 系统具体存储了什么数据？底层是如何组织和存储的？
...
### 4.2 基础问题：为什么代码 RAG 不能直接用通用文本 RAG 方案？
...
### 4.3 基础问题：Embedding 模型在项目中应该怎么选，选错会出现什么现象？
...
### 4.4 进阶问题：Embedding 切 Chunk 应该怎么做，为什么固定长度切分经常不够？
...
### 4.5 深度问题：硬切分（固定长度切分）在实际代码场景中有哪些致命弊端？
...
### 4.6 进阶问题：混合检索（Dense + Sparse）是如何工作的？为什么需要两种检索方式？
...
### 4.7 深度问题：如何在 100K chunks 上实现 p95 < 50ms 的检索延迟？
...
### 4.8 核心架构问题：RAG 是如何切块（Chunking）的？
...
### 4.9 核心架构问题：当前的 RAG 是如何进行分层（Layering）的？
...
### 8.1 架构问题：ReAct 与 Planning 的关系？什么时候需要先规划再执行？
...
### 8.2 架构问题：DAG 执行中如何处理节点失败？重试、跳过还是回滚？
...
### 8.3 架构问题：如何实现 DAG 的动态调整？执行中发现新依赖怎么办？
...
## 九、Multi-Agent 协作与工具自学习

### 9.1 Multi-Agent 协作：如何让多个 Agent 并行工作且不冲突？
...
### 9.2 工具自学习（Tool Learning）：Agent 如何从历史执行中优化工具选择？
...
### 9.3 Skill Registry：如何实现动态工具注册与 Prompt Cache 优化？
...
## 十一、记忆系统（Memory System）架构设计

在真实的 Agent 交互中，仅仅依靠上下文窗口是不够的。我们需要一套完整的记忆层级来支撑长程复杂任务。

### 11.1 核心架构问题：如何设计 Agent 的三层记忆系统（Memory System）？上下文窗口不够用怎么办？
...
### 11.2 核心架构问题：当前三层记忆之间是如何进行转化的？什么时候进行转化？如何进行转化？
...
### 11.3 核心架构问题：当前三层记忆是如何加载到上下文中的？什么时候加载？加载哪些内容？
...
### 11.4 基础问题：你们的长期记忆系统为什么采用 Hybrid Memory（冷热分离）架构？
...
### 11.5 进阶问题：长期记忆是如何从对话流水中自动提取的？（Extractor 原理）
...
### 11.6 深度问题：如果在入库时发现新提取的记忆与老记忆存在冲突，系统是如何处理的？
...
### 11.7 细节追问：到达上下文上限触发蒸馏长期记忆时，这个过程是同步的还是异步的？
...
### 12.1 上下文管理系统在整个 Code Agent 系统中的位置和作用
...
## 十三、安全边界与意图冲突处理（Safety & Intent Conflict Resolution）

### 13.1 深度问题：如果用户和 AI 对话生成的指令本身有冲突，系统应该怎么处理？
